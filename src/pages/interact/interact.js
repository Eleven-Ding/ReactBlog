...
export default memo(function Interact() {
    ...
    return ( <
        ...
        <
        Input value = { friendTitle }
        className = "input"
        onChange = {
            (e) => setFriendTitle(e.target.value)
        }
        size = "large"
        placeholder = "大佬的网站名字" /
        >
        ...
        <
        div style = {
            { color: homeFontColor, fontSize: "20px" }
        } > 本站格式 < /div> <
        ...
        hr className = "parting-line" / >
        <
        div className = "dsy_tip" > 赶快来留言吖, 博主会经常光顾本站 < span style = {
            { color: "#ec5328" }
        } > (支持markdown语法) < /span></div >
        <
        div style = {
            {
                display: "flex",
        ...
        article_id = {-1 } > < /Comment> <
        p style = {
            { textAlign: "center", color: "#1890FF ", marginTop: "20px", cursor: "pointer" }
        }
        onClick = {
            () => showMoreComment()
        } >
