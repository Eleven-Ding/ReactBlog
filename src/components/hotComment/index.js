          ...
          hotComments.map((item, index) => {
            return (
              <Comment
                ...
                actions={actions}
                author={<a style={{ color: "#1890FF" }}>{item.username}</a>}
                avatar={<Avatar src={item.qqurl} alt={item.username} />}
                content={<p style={{ color: "#7B7B7B" }}>{item.comment}</p>}
                datetime={
                  <Tooltip key="comment-basic-like" title="点赞次数">
                    <span style={{ fontSize: "13px", color: "deeppink" }}>
